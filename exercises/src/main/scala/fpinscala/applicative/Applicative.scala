...
object Monad {
  ...
  def composeM[F[_],N[_]](implicit F: Monad[F], N: Monad[N], T: Traverse[N]):
    ...
    new Monad[({type f[x] = F[N[x]]})#f] {
      def unit[A](a: => A) = F.unit(N.unit(a))
...
object Applicative {
  ...
  def validationApplicative[E]: Applicative[({type f[x] = Validation[E,x]})#f] =
    new Applicative[({type f[x] = Validation[E,x]})#f] {
      ...
      override def map2[A, B, C](va: Validation[E, A], vb: Validation[E, B])(f: (A, B) => C):
      Validation[E, C] =
        (va, vb) match {
...
trait Traverse[F[_]] extends Functor[F] with Foldable[F] {
  ...
  def compose[G[_]](implicit G: Traverse[G]): Traverse[({type f[x] = F[G[x]]})#f] = {
    ...
    new Traverse[({type f[x] = F[G[x]]})#f] {
      override def traverse[H[_]:Applicative, A, B](as: F[G[A]])(f: A => H[B]): H[F[G[B]]] =
        self.traverse[H,G[A],G[B]](as)(ga => G.traverse[H,A,B](ga)(f))
    }
  }
}
...
object Traverse {
  ...
  val treeTraverse = new Traverse[Tree] {
    override def traverse[G[_],A,B](ta: Tree[A])(f: A => G[B])(implicit G: Applicative[G]): G[Tree[B]] = {
      val children = listTraverse.traverse(ta.tail)(t => traverse(t)(f))
      G.map2(f(ta.head), children)(Tree(_, _))
    }
  }
}
